            ...
            authKeyPromise.then(function (auth) {
                ...
                askPhoneNumber().then(function (phoneNumber) {
                    ...
                    clientProxy.sendCodeToPhone(phoneNumber).then(function(result){
                        ...
                        console.log('send code error: ' + error.stack);
                    });
                });
            });
        });
            ...
            switch (passwordType) {
                ...
                case PASSWORD_TYPE.STRONG:
                    ...
                    (function askStrong() {
                        ...
                        //ui.askPasswordInput(i18n.signUp.choose_password_strong, /^.{10,}$/).then(callback, askStrong);
                    })();
                    break;
                default:
                    askType();
...
function askPhoneNumber() {
    return new Promise(function (fulfill) {
        ...
        (function askNumber() {
            ui.askNumericInput(i18n.signUp.give_phoneNumber).then(function (phoneNumber) {
                if (phoneNumber.length < 3 || (phoneNumber[0] !== '0' && phoneNumber[1] !== '0')) {
                    console.log(i18n.signUp.give_phoneNumber_error);
                    ui.spacer();
                    askNumber();
                } else {
                    fulfill(phoneNumber);
