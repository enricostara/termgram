...
function askPhoneNumberToSendCode() {
    return new Promise(function (fulfill, reject) {
        ...
        (function askPhone() {
            ui.askPhoneInput(i18n.signUp.give_phoneNumber).then(function (phoneNumber) {
                console.log('phoneNumber: %s', phoneNumber);
                clientProxy.sendCodeToPhone(phoneNumber).then(fulfill, function (error) {
                    if (error.message == 400) {
                        console.log(i18n.signUp.give_phoneNumber_invalid);
                        askPhone();
                    } else {
                        reject(error);
                    }
