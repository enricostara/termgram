...
function sendCodeToPhone(phoneNumber) {
    return new Promise(function (fulfill, reject) {
        ...
        client.auth.sendCode(phoneNumber, function (result) {
...
function signUp(phoneNumber, codeHash, code, firstName, lastName) {
    return new Promise(function (fulfill, reject) {
        client.once('error', reject);
        client.auth.signUp(phoneNumber, codeHash, code, firstName, lastName, function (result) {
            if ('mtproto.type.Rpc_error' == result.typeName) {
                reject(new Error(result.error_message));
            } else {
                fulfill(result);
...
function getContacts() {
    return new Promise(function (fulfill, reject) {
        client.once('error', reject);
        client.contacts.getContacts('', function (result) {
            if ('mtproto.type.Rpc_error' == result.typeName) {
                reject(new Error(result.error_message));
            } else {
                fulfill(result);
